...
class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        return {
          status: 'error',
          message: raw?.detail || raw?.message || `HTTP ${response.status}`,
        };
      }

      const unwrapped = raw && typeof raw === 'object' && 'data' in raw ? (raw as any).data : raw;
      const message = raw && typeof raw === 'object' && 'message' in raw ? (raw as any).message : undefined;
  ...
  async uploadResume(file: File): Promise<ApiResponse<ResumeUploadResponse>> {
    return this.uploadFile('/api/v1/resumes/upload', file);
  }
  ...
  async getAISuggestions(resumeText: string, targetRole?: string): Promise<ApiResponse<AISuggestionsResponse>> {
    // For testing without authentication, try both authenticated and non-authenticated requests
    try {
      // First try with authentication if we have a token
      if (this.token) {
        const response = await this.request<AISuggestionsResponse>('/api/v1/ai-suggestions', {
          method: 'POST',
          body: JSON.stringify({ 
        ...
        // If authenticated request fails with auth error, try without auth
        if (response.message?.includes('authenticated') || response.message?.includes('token')) {
          console.warn('Authenticated request failed, trying without auth...');
        } else {
          return response; // Return other types of errors
        }
      }
      
      // Try without authentication (for testing) - use public endpoint
      const response = await fetch(`${this.baseUrl}/api/v1/ai-suggestions-public`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async getRecentEvaluations(): Promise<ApiResponse> {
    return this.request('/api/v1/dashboard/recent-evaluations');
  }
